...
public class PhotoViewAttacher implements IPhotoView, View.OnTouchListener, OnGestureListener,
	...
	public boolean onTouch(View v, MotionEvent ev) {
		...
		if (mZoomEnabled && (hasDrawable((ImageView) v)||null!=mMovie)) {
	...
	public void setZoomable(boolean zoomable,Movie movie){
	...
	private RectF getDisplayRect(Matrix matrix) {
		...
		if (null != imageView) {
			...
			if (null != mMovie) {
				mDisplayRect.set(0, 0, mMovie.width(), mMovie.height());
				matrix.mapRect(mDisplayRect);
				return mDisplayRect;
			}
		}
	...
	private void updateBaseMatrix(Drawable d) {
		ImageView imageView = getImageView();
		if (null == imageView || (null == d&&null==mMovie)) {
			return;
		}
		final float viewWidth = getImageViewWidth(imageView);
		final float viewHeight = getImageViewHeight(imageView);
		final int drawableWidth = null==mMovie?d.getIntrinsicWidth():mMovie.width();
		final int drawableHeight = null==mMovie?d.getIntrinsicHeight():mMovie.height();
		mBaseMatrix.reset();
